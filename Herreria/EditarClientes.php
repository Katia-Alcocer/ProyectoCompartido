...
// Obtener datos del cliente junto con domicilio (suponiendo que tienes tabla Domicilios)
$sql = "SELECT c.*, p.Nombre, p.Paterno, p.Materno, p.Telefono, p.Email, p.Edad, p.Sexo, p.idDomicilio,
...
$sqlCategorias = "SELECT idCategoria, Categoria FROM Descuentos ORDER BY Categoria";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $idDomicilio = (int)$cliente['idDomicilio']; // Mantener el idDomicilio actual
    ...
    // Buscar c_CP según d_CP recibido
    ...
    if (!$error) {
        // Preparar llamada al procedimiento almacenado con todos los parámetros
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="EditarCliente.css" />
                            ...
                            <select name="idDescuento" required>
                                ...
                                <?php foreach ($categorias as $cat): ?>
                                    <option value="<?php echo $cat['idCategoria']; ?>" 
                                        <?php echo ($cliente['idDescuento'] == $cat['idCategoria']) ? 'selected' : ''; ?>>
